...
use druid::widget::{ControllerHost,SizedBox,
...
pub fn make_goto_ui() -> impl Widget<PdfViewState> {
    SizedBox::new(
        ...
        .with_default_spacer()
        .with_child(Label::new(" using page offset (which file page (visible in the window title) does page number 1 appears on?"))
        .with_default_spacer()
...
impl<W: Widget<PdfViewState>> Controller<PdfViewState, W> for GotoController {
    fn event(
        ...
        match event {
            ...
            Event::KeyDown(e) => {
                if e.key == Key::Escape || e.key == Key::Enter || e.key == Key::Character(" ".to_string())  {
                    ctx.resign_focus();
